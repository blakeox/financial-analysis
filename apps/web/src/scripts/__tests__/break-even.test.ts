...
function calculateBreakEven(input: BreakEvenInput): BreakEvenResult {
  const { fixedCosts, variableCostPerUnit, sellingPricePerUnit, currentSalesUnits, targetProfit } =
    input;
...
describe('Break-Even Analysis Calculator', () => {
  ...
  describe('Sensitivity Analysis', () => {
    ...
    it('should calculate impact of 10% price decrease', () => {
      ...
      expect(result.sensitivity.price10PercentDecrease.units).toBeGreaterThan(
        result.breakEven.units
      );
    });
    ...
    it('should calculate impact of 10% cost increase', () => {
      ...
      expect(result.sensitivity.costs10PercentIncrease.units).toBeGreaterThan(
        result.breakEven.units
      );
    });
  });
