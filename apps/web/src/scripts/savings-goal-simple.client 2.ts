...
const displayResults = (result: SavingsGoalResults): void => {
  ...
  resultsContainer.innerHTML = `
    ...
    <div class="bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-lg p-6 mb-6 border border-purple-200 dark:border-purple-700">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">Track your progress toward your goal</p>
        ...
        ${result.milestones.map((milestone, idx) => {
          ...
          return `
            <div class="bg-white dark:bg-gray-800 rounded-lg p-3 border-2 ${achieved ? `border-${color}-500` : 'border-gray-200 dark:border-gray-700'}">
              ...
              <p class="text-xs text-gray-600 dark:text-gray-400 mb-1">${formatCurrency(milestone.amount)}</p>
              <p class="text-xs text-gray-500 dark:text-gray-400">${milestone.date}</p>
              <p class="text-xs text-gray-500 dark:text-gray-400">${milestone.months} months</p>
            </div>
          `;
        }).join('')}
      </div>
...
const parseSavingsGoalInput = (form: HTMLFormElement): SavingsGoalInputs => {
  ...
  const monthlyContribution = calculator.calculateMonthlyContribution(
    ...
    Number.isNaN(inflationRate) ? 3 / 100 : inflationRate / 100
  );
  ...
  return {
    ...
    inflationRate: Number.isNaN(inflationRate) ? 3 / 100 : inflationRate / 100,
    goalType,
    monthlyContribution,
  };
};
...
const initSavingsGoalPage = (): void => {
  ...
  form.addEventListener('submit', (event) => {
    handleCalculatorSubmission(event, {
      ...
      calculate: (inputs) => new SimpleSavingsGoalCalculator().calculate(inputs),
