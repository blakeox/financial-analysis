...
// Note: business-loan-qualifier uses percentage rates (e.g., 8.5 for 8.5%)
// So we need to divide by 100 when calling the shared function
function calculateMonthlyPayment(principal: number, annualRatePercent: number, years: number): number {
  return calculateMonthlyPaymentSimple(principal, annualRatePercent / 100, years * 12);
}
