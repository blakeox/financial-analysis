...
function evaluateBankTermLoan(
  input: LoanQualifierInput,
  dscr: number,
  ltv: number
): LoanEligibility {
  const requirements: string[] = [];
...
function generateRecommendations(
  ...
  if (eligibleCount === 0) {
    recs.push(
      "🚨 You don't currently qualify for traditional financing. Consider improving DSCR, credit score, or business age first."
    );
  } else if (eligibleCount === 1) {
    recs.push(
      '✓ You qualify for limited options. Improving DSCR or credit score would unlock more favorable terms.'
    );
  } else {
    recs.push(
      '✓ You qualify for multiple loan types. Compare rates and terms to find the best fit.'
    );
  }
  ...
  if (input.creditScore < 720) {
    recs.push(
      '📈 Improving credit score above 720 would qualify you for better rates (potentially 0.5-1% lower).'
    );
  }

  if (ltv > 80 && input.collateralValue > 0) {
    recs.push(
      '💰 Consider increasing down payment to reduce LTV below 80% for better approval odds.'
    );
  }

  if (input.businessAge < 2) {
    recs.push(
      '⏳ Operating for 2+ years significantly improves approval odds and unlocks SBA programs.'
    );
  }
...
function displayResults(result: LoanQualifierResult, input: LoanQualifierInput): void {
  ...
  summaryCards.innerHTML = `
    <div class="bg-violet-50 dark:bg-violet-900/20 rounded-lg p-4">
      <h5 class="text-sm font-medium text-violet-900 dark:text-violet-100">DSCR</h5>
      <p class="text-2xl font-bold ${result.dscr >= 1.25 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}">${result.dscr.toFixed(2)}</p>
      <p class="text-xs text-violet-700 dark:text-violet-300 mt-1">${result.dscr >= 1.25 ? '✓ Strong' : '❌ Too low'}</p>
    </div>
    <div class="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-4">
      <h5 class="text-sm font-medium text-emerald-900 dark:text-emerald-100">Qualified Loans</h5>
      <p class="text-2xl font-bold text-emerald-600 dark:text-emerald-400">${eligibleLoans.length}/4</p>
      <p class="text-xs text-emerald-700 dark:text-emerald-300 mt-1">loan types available</p>
    </div>
    <div class="bg-violet-50 dark:bg-violet-900/20 rounded-lg p-4">
      <h5 class="text-sm font-medium text-violet-900 dark:text-violet-100">Best Option</h5>
      <p class="text-lg font-bold text-violet-600 dark:text-violet-400">${result.bestOption}</p>
      <p class="text-xs text-violet-700 dark:text-violet-300 mt-1">${eligibleLoans.length > 0 ? 'Recommended' : 'Improve financials'}</p>
    </div>
  ...
  resultsContainer.innerHTML = `
    ...
    <div class="bg-white/90 dark:bg-slate-950/40 rounded-lg shadow-md p-6 mb-6">
      ...
      <div class="space-y-4">
        ${Object.values(result.loanEligibility)
          .map(
            (loan) => `
          <div class="border-2 ${loan.eligible ? 'border-emerald-500' : 'border-slate-300 dark:border-slate-700'} rounded-lg p-4">
            <div class="flex justify-between items-start mb-3">
              ...
              <div class="text-right">
                ...
                <p class="text-lg font-bold ${loan.approvalOddsPercent >= 70 ? 'text-emerald-600 dark:text-emerald-400' : loan.approvalOddsPercent >= 50 ? 'text-yellow-600 dark:text-yellow-400' : 'text-rose-600 dark:text-rose-400'}">${loan.approvalOddsPercent}%</p>
              </div>
            </div>
...
function parseFormInput(form: HTMLFormElement): LoanQualifierInput {
  ...
  return {
    ...
    loanPurpose: ((formData.get('loanPurpose') as string) ||
      'working-capital') as LoanQualifierInput['loanPurpose'],
...
function validateInput(input: LoanQualifierInput): void {
  ...
  if (input.netIncome <= 0)
    throw new Error('Net income must be positive (business must be profitable)');
  if (input.creditScore < 300 || input.creditScore > 850)
    throw new Error('Credit score must be 300-850');
}
