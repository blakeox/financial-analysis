  ...
  const html = pricing
    .map((tier) => {
      ...
      const overageMarkup = overagePerRequest > 0
        ? `<p class="mt-4 text-sm text-gray-500 text-center">Overage: $${overagePerRequest}/request</p>`
        : '';
        ...
        <div class="relative bg-white rounded-2xl shadow-xl ${isPopular ? 'ring-2 ring-indigo-600 scale-105' : ''}">
          ...
          <div class="p-8">
            ...
            <div class="mt-4 flex items-baseline">
              <span class="text-5xl font-extrabold text-gray-900">$${price}</span>
              ${price > 0 ? '<span class="ml-2 text-gray-500">/month</span>' : '<span class="ml-2 text-gray-500">Forever</span>'}
            </div>
            <ul class="mt-8 space-y-4">
              <li class="flex items-start">
                ...
                <span class="text-gray-700">${monthlyQuota.toLocaleString()} requests/month</span>
              </li>
              <li class="flex items-start">
                <svg class="h-6 w-6 text-green-500 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
                <span class="text-gray-700">${rateLimitPerSec} requests/second</span>
              </li>
              <li class="flex items-start">
                <svg class="h-6 w-6 text-green-500 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
                <span class="text-gray-700">All analysis endpoints</span>
              </li>
              <li class="flex items-start">
                <svg class="h-6 w-6 text-green-500 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
                <span class="text-gray-700">${supportLevel} support</span>
              </li>
              ${safeTier === 'enterprise'
                ? '<li class="flex items-start"><svg class="h-6 w-6 text-green-500 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg><span class="text-gray-700">99.9% SLA</span></li>'
                : ''}
            </ul>
