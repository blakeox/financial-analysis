...
const renderPricingCards = (pricing: PricingTier[]) => {
  ...
  const html = pricing
    .map((tier) => {
      ...
      const overageMarkup =
        overagePerRequest > 0
          ? `<p class="mt-4 text-center text-sm fa-meta-copy">Overage: $${overagePerRequest}/request</p>`
          : '';
        ...
        <div class="fa-card relative h-full ${isPopular ? 'ring-2 ring-violet-500 -translate-y-1' : ''}">
          ...
          <div class="p-8">
            ...
            <div class="mt-4 flex items-baseline">
              ...
              ${price > 0 ? '<span class="ml-2 fa-copy-muted">/month</span>' : '<span class="ml-2 fa-copy-muted">Forever</span>'}
            </div>
