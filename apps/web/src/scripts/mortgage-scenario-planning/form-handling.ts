...
import type { MortgageScenarioPlanningInput, ScenarioInput } from './types';
import { MIN_SCENARIOS, MAX_SCENARIOS, SCENARIO_COLORS } from './constants';
// generateScenarioName moved to calculations module
import { coerceNumber, showError } from '../../utils/calculator-utilities';
...
export function injectDynamicScenarioUI(form: HTMLFormElement, initialCount?: number): void {
  ...
  if (
    ...
    initialCount >= MIN_SCENARIOS &&
    initialCount <= MAX_SCENARIOS
  ) {
  ...
  const optionAGroup =
    form.querySelector('[data-group-name*="Option A"]') ||
    form.querySelector('.form-group:has(#scenario1Down)');
  const optionBGroup =
    form.querySelector('[data-group-name*="Option B"]') ||
...
export function setupFormEventListeners(form: HTMLFormElement): void {
  ...
  const loanTermSelect = form.querySelector<HTMLSelectElement>(
    '[name="loanTerm"], [name="loanTermYears"]'
  );
  if (loanTermSelect) {
