...
export function createScenarioCard(index: number): HTMLElement {
  ...
  card.className = `scenario-card bg-white dark:bg-gray-800 rounded-lg border-2 border-${colors.bg}-200 dark:border-${colors.bg}-700 p-4 relative`;
  ...
  card.innerHTML = `
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-2">
        ...
        <h3 class="font-semibold text-gray-900 dark:text-white">
          Scenario ${letter}
          <span class="text-sm font-normal text-gray-500 dark:text-gray-400">${index === 0 ? '(Conservative)' : index === 1 ? '(Alternative)' : ''}</span>
        </h3>
      </div>
      ${index >= MIN_SCENARIOS ? `
        <button type="button" 
          class="remove-scenario-btn p-1.5 rounded-full hover:bg-red-100 dark:hover:bg-red-900/30 text-red-500 transition-colors" 
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label for="scenario${index}Down" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          Down Payment
          <span class="text-xs text-gray-500 ml-1">(20%+ avoids PMI)</span>
        </label>
        <div class="relative">
          ...
          <input type="number" 
            ...
            class="w-full pl-7 pr-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-${colors.bg}-500 focus:border-transparent"
            min="0" 
            step="1000" 
      ...
      <div>
        <label for="scenario${index}Rate" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          Interest Rate
          <span class="text-xs text-gray-500 ml-1">(Shop 3-5 lenders)</span>
        </label>
        <div class="relative">
          <input type="number" 
            id="scenario${index}Rate" 
            name="scenario${index}Rate"
            class="w-full pl-3 pr-7 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-${colors.bg}-500 focus:border-transparent"
            min="0" 
            max="30" 
            step="0.01" 
      ...
      <div>
        <label for="scenario${index}Extra" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
      ...
      <div>
        <label for="scenario${index}Closing" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          Closing Costs
          <span class="text-xs text-gray-500 ml-1">(typically 2-5%)</span>
        </label>
        <div class="relative">
          <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span>
          <input type="number" 
...
function addScenarioButton(_form: HTMLFormElement, container: HTMLElement): void {
  ...
  addBtn.className = 'w-full py-3 px-4 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-gray-600 dark:text-gray-400 hover:border-blue-500 hover:text-blue-500 dark:hover:border-blue-400 dark:hover:text-blue-400 transition-colors flex items-center justify-center gap-2 font-medium';
  addBtn.innerHTML = `
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
    </svg>
    <span>Add Another Scenario</span>
    <span class="text-xs text-gray-400">(${scenarioCount}/${MAX_SCENARIOS})</span>
  `;
