...
export function renderPayoffTimelineChart(scenarios: Scenario[]): string {
  ...
  setTimeout(() => {
    ...
    scenarios.forEach((scenario, idx) => {
      ...
      ctx.textAlign = 'left';
      const label =
        scenario.name.length > 15 ? scenario.name.substring(0, 15) + '...' : scenario.name;
...
export function renderEquityGrowthChart(scenarios: Scenario[]): string {
  ...
  setTimeout(() => {
    ...
    equityData.forEach((data, idx) => {
      ...
      ctx.font = '10px system-ui, sans-serif';
      const label =
        data.scenario.name.length > 10
          ? data.scenario.name.substring(0, 10) + '...'
          : data.scenario.name;
