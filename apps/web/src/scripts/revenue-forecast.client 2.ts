...
import { formatCurrency, parseNumber, showResults, hideError, showError, setLoadingState } from '../utils/calculator-utilities';
import { autoInitCalculator } from '../utils/calculator-init';
import { collectRevenueStreams as collectRevenueStreamsUtil } from '../utils/form-collection';
 ...
 */
export const collectRevenueStreams = collectRevenueStreamsUtil;
...
export const initRevenueForecastCalculator = (): void => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      hideError();
      if (calculateBtn) {
        calculateBtn.textContent = 'Forecasting...'; // Custom text
      }
      ...
      const errMsg = error instanceof Error ? error.message : 'An error occurred during calculation';
      showError(errMsg);
    } finally {
      // Re-enable button
      setLoadingState(calculateBtn, false);
      if (calculateBtn) {
        calculateBtn.textContent = 'Generate Forecast'; // Restore custom text
      }
    }
  });
...
autoInitCalculator(
  ['/revenue-forecast', '/calculator/revenue-forecast'],
