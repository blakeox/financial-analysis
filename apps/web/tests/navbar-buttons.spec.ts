...
test.describe('Navbar buttons functionality', () => {
  ...
  test('desktop navigation links are functional', async ({ page }) => {
    ...
    // Test Models link
    const modelsLink = desktopNav.locator('a[data-testid="nav-desktop-link"]', { hasText: 'Models' });
    ...
    // Test Analysis link
    const analysisLink = desktopNav.locator('a[data-testid="nav-desktop-link"]', { hasText: 'Analysis' });
  ...
  test('navbar styling and layout integrity', async ({ page }) => {
    ...
    const themeToggleBorderRadius = await themeToggle.evaluate(el => getComputedStyle(el).borderRadius);
