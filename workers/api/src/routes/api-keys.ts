...
export async function createApiKey(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const metadata = validated.description ? JSON.stringify({ description: validated.description }) : null;
...
export async function getKeyUsage(keyId: string, env: Env): Promise<Response> {
  ...
  try {
    ...
    `).bind(parseInt(keyId, 10), yearMonth).first<{
    ...
    const recent = await env.DB.prepare(`
      ...
      ORDER BY created_at DESC
      LIMIT 100
    `).bind(parseInt(keyId, 10), oneDayAgo).all<{
      endpoint: string;
      method: string;
      status_code: number;
