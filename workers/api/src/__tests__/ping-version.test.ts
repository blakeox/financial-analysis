...
function makeEnv(commit?: string) {
  const env: { ENVIRONMENT: string; DB: D1Database; SESSIONS: KVNamespace; DOCUMENTS: R2Bucket; COMMIT_SHA?: string } = {
    ENVIRONMENT: 'test',
...
describe('/version', () => {
  it('returns version metadata with commit when set', async () => {
    ...
    const res = await api.fetch(req, env, ctx);
    expect(res.status).toBe(200);
    const json = (await res.json()) as { commit: string; environment: string; service: string; version: string; timestamp: string };
