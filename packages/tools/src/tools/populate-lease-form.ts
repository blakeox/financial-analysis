  ...
  static execute(input: unknown): Promise<{ values: Record<string, unknown>; message: string }> {
    try {
      ...
      if (validated.securityDeposit !== undefined)
        values.securityDeposit = validated.securityDeposit;
      ...
      if (validated.pricePerSquareFoot !== undefined)
        values.pricePerSquareFoot = validated.pricePerSquareFoot;
      ...
      return Promise.reject(
        new Error(
          `Failed to populate lease form: ${error instanceof Error ? error.message : String(error)}`
        )
      );
    }
  }
