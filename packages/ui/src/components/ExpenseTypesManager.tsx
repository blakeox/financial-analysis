...
export function ExpenseTypesManager({
  ...
  const updateExpenseType = (
    id: string,
    field: keyof ExpenseTypeData,
    value: string | number | boolean
  ) => {
    onChange(expenseTypes.map((exp) => (exp.id === id ? { ...exp, [field]: value } : exp)));
  };

  const totalMonthlyExpenses = expenseTypes.reduce(
    (sum, exp) => sum + (exp.isActive ? exp.currentMonthlyAmount : 0),
    0
  );

  const categorySummary = expenseTypes.reduce(
    (acc, exp) => {
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <div
            className={cn(
              summaryCardBase,
              'border-rose-200 bg-rose-50/90 dark:border-rose-900/70 dark:bg-rose-950/30'
            )}
          >
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateExpenseType(expense.id, 'growthRate', parsers.percentage(e.target.value))
                  }
                  disabled={readonly}
                  min="-100"
                  max="100"
        ...
        {!readonly && (
          <div className="rounded-[1.5rem] border-2 border-dashed border-slate-300 bg-slate-50/70 p-5 dark:border-slate-700 dark:bg-slate-900/60">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewExpense({
                    ...newExpense,
                    currentMonthlyAmount: parsers.number(e.target.value),
                  })
                }
                placeholder="0"
                min="0"
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewExpense({ ...newExpense, growthRate: parsers.percentage(e.target.value) })
                }
                placeholder="0"
                min="-100"
                max="100"
            ...
            <div className="mt-3">
              <Button onClick={addExpenseType} disabled={!newExpense.name}>
                Add Expense Type
              </Button>
            </div>
          </div>
