...
describe('OptionsPricingInputSchema', () => {
  ...
  it('requires Bermudan exercise dates', () => {
    ...
    if (!result.success) {
      expect(result.error.issues[0].message).toContain(
        'Bermudan options require bermudanExerciseDates'
      );
    }

    const parsed = OptionsPricingInputSchema.parse(
      buildInput({
  ...
  it('requires marketPrice for implied volatility', () => {
    const result = OptionsPricingInputSchema.safeParse(
      buildInput({ calculateImpliedVolatility: true })
    );
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues[0].message).toContain(
        'Implied volatility calculation requires marketPrice'
      );
    }
  ...
  it('validates price range ordering', () => {
    const invalid = OptionsPricingInputSchema.safeParse(
      buildInput({
