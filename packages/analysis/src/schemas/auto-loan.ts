...
export const AutoLoanInputSchema = z.object({
  ...
  downPayment: z.number().nonnegative('Down payment cannot be negative').default(0),
  tradeInValue: z.number().nonnegative('Trade-in value cannot be negative').default(0),
  tradeInOwed: z.number().nonnegative('Amount owed on trade-in cannot be negative').default(0),
  ...
  salesTaxRate: z
    ...
    .default(0),
  registrationFees: z.number().nonnegative('Registration fees cannot be negative').default(0),
  dealerFees: z.number().nonnegative('Dealer fees cannot be negative').default(0),
  ...
  manufacturerRebate: z.number().nonnegative('Manufacturer rebate cannot be negative').default(0),

  // Payment options
  includeGapInsurance: z.boolean().default(false),
  gapInsuranceCost: z.number().nonnegative('GAP insurance cost cannot be negative').default(0),
