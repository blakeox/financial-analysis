...
export class BusinessValuationEngine {
  ...
  static analyze(input: BusinessValuationInput): BusinessValuationResult {
    ...
    const ebitdaMargin = input.annualRevenue > 0 ? (input.annualEbitda / input.annualRevenue) * 100 : 0;
    const netMargin = input.annualRevenue > 0 ? (input.annualNetIncome / input.annualRevenue) * 100 : 0;
    ...
    const mostRelevantMethod = methods.length > 0 
      ? methods.reduce((best, current) => 
          current.weight > best.weight ? current : best
        )
      : null;
    
    // Calculate per-unit metrics
    const valuePerCustomer = input.customerCount && input.customerCount > 0
      ? valuationMid / input.customerCount
      : undefined;
    const valuePerDollarRevenue = input.annualRevenue > 0 
      ? valuationMid / input.annualRevenue 
      : 0;
    ...
    return {
      ...
      ebitdaMultiple: methods.find(m => m.name === 'EBITDA Multiple') 
        ? input.annualEbitda > 0 ? methods.find(m => m.name === 'EBITDA Multiple')!.value / input.annualEbitda : 0
        : 0,
      revenueMultiple: methods.find(m => m.name === 'Revenue Multiple')
        ? input.annualRevenue > 0 ? methods.find(m => m.name === 'Revenue Multiple')!.value / input.annualRevenue : 0
        : 0,
      sdeMultiple: methods.find(m => m.name === 'SDE Multiple')
        ? sde > 0 ? methods.find(m => m.name === 'SDE Multiple')!.value / sde : 0
        : 0,
      bookValue,
   ...
   */
  private static getIndustryMultiples(industry: string): { ebitda: number; revenue: number; sde: number } {
  ...
  private static calculateSDE(input: BusinessValuationInput): number {
    ...
    const estimatedOwnerComp = Math.max(100000, input.annualRevenue * 0.20);
   ...
   */
  private static calculateAdjustedBookValue(input: BusinessValuationInput, bookValue: number): number {
  ...
  private static generateInsights(
    ...
    insights.push(`Based on ${methods.length} valuation methods, your business is worth approximately ${this.formatCurrency(value)}.`);
    ...
    if (ebitdaMethod) {
      ...
      insights.push(`EBITDA multiple of ${multiple.toFixed(2)}x is ${multiple >= 5 ? 'strong' : multiple >= 3 ? 'typical' : 'below average'} for ${input.industry} businesses.`);
    }
    ...
    if (revenueMethod) {
      ...
      insights.push(`Revenue multiple of ${multiple.toFixed(2)}x reflects ${multiple >= 2 ? 'high growth potential' : 'stable but moderate growth expectations'}.`);
    }
    ...
    if (positiveAdj.length > 0) {
      ...
      insights.push(`✅ Positive factors add ${totalPositive.toFixed(0)}% to base valuation: ${positiveAdj.map(a => a.name).join(', ')}.`);
    }
    ...
    if (negativeAdj.length > 0) {
      ...
      insights.push(`⚠️ Risk factors reduce valuation by ${totalNegative.toFixed(0)}%: ${negativeAdj.map(a => a.name).join(', ')}.`);
    }
  ...
  private static generateRecommendations(
    ...
    // EBITDA improvement
    if (ebitdaMargin < 15) {
      recommendations.push(`📈 Improve EBITDA margin from ${ebitdaMargin.toFixed(1)}% to 20%+ by reducing costs or increasing prices.`);
      recommendations.push(`   • For every 5% margin improvement, valuation could increase by ${this.formatCurrency(input.annualRevenue * 0.05 * 4)}`);
    }
    ...
    // Risk mitigation
    const hasHighRisk = adjustments.some(a => 
      a.impact === 'negative' && Math.abs(a.adjustmentPercent) >= 20
    );
    
    if (hasHighRisk) {
      recommendations.push(`🛡️ Address key risk factors to unlock 20-40% more value:`);
      adjustments.filter(a => a.impact === 'negative').forEach(adj => {
    ...
    // Documentation
    if (!input.hasDocumentedProcesses) {
      recommendations.push(`📝 Document all processes and procedures - adds 10-15% to valuation and makes sale easier.`);
    }
    
    // Recurring revenue
    if (!input.hasRecurringRevenue) {
      recommendations.push(`🔄 Add recurring revenue streams (subscriptions, contracts, retainers) - can increase valuation by 20-30%.`);
    }
    
    // Customer diversification
    if (input.customerConcentration && input.customerConcentration > 20) {
      recommendations.push(`👥 Reduce customer concentration from ${input.customerConcentration.toFixed(0)}% by acquiring more customers.`);
  ...
  private static generateWarnings(input: BusinessValuationInput, value: number): string[] {
    ...
    if (input.annualEbitda < 0) {
      warnings.push(`🚨 Negative EBITDA makes traditional valuation difficult. Focus on path to profitability.`);
    }
    ...
    if (input.customerConcentration && input.customerConcentration > 50) {
      warnings.push(`🚨 CRITICAL: ${input.customerConcentration.toFixed(0)}% revenue concentration creates extreme risk. Losing top customer would be catastrophic.`);
    }
    
    if (value < 100000 && input.annualRevenue > 500000) {
      warnings.push(`⚠️ Low valuation relative to revenue suggests profitability issues or other concerns.`);
    }
