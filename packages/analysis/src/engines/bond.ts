...
export class BondAnalyzer {
  ...
  static analyze(input: BondAnalysisInput): BondPricingResult {
    ...
    const marketYield = this.getMarketYieldForBond(
      bond,
      marketData.yieldCurve,
      marketData.creditSpread
    );
    ...
    const yieldToMaturity = bond.currentPrice
      ? this.calculateYTM(bond, bond.currentPrice)
      : marketYield;
    const yieldToCall =
      bond.callableDate && bond.callPrice
        ? this.calculateYTC(bond, price, bond.callableDate, bond.callPrice)
        : undefined;
    const yieldToPut =
      bond.puttableDate && bond.putPrice
    ...
    const effectiveConvexity = this.calculateEffectiveConvexity(
      bond,
      yieldToMaturity,
      marketData.volatility
    );
    ...
    if (
      analysis.includeOptionAnalysis &&
      (bond.bondType === 'callable' || bond.bondType === 'puttable')
    ) {
  ...
  private static calculateConvexity(bond: Bond, yieldRate: number): number {
    ...
    return (
      convexitySum / totalPresentValue / Math.pow(1 + periodYield, 2) / Math.pow(periodsPerYear, 2)
    );
  }
   ...
   */
  private static calculateEffectiveDuration(
    bond: Bond,
    yieldRate: number,
    _volatility: number
  ): number {
   ...
   */
  private static calculateEffectiveConvexity(
    bond: Bond,
    yieldRate: number,
    _volatility: number
  ): number {
  ...
  private static calculateZSpread(bond: Bond, yieldCurve: YieldCurve, price: number): number {
    ...
    for (let i = 0; i < maxIterations; i++) {
      ...
      const priceWithDelta = this.calculateBondPriceWithZSpread(
        bond,
        yieldCurve,
        zSpread + deltaSpread
      );
  ...
  private static analyzeEmbeddedOptions(
    ...
    if (bond.bondType === 'callable' && bond.callPrice && bond.callableDate) {
      ...
      const d1 =
        (Math.log(straightBondPrice / bond.callPrice) +
    ...
    if (bond.bondType === 'puttable' && bond.putPrice && bond.puttableDate) {
      ...
      const d1 =
        (Math.log(straightBondPrice / bond.putPrice) +
      ...
      optionValue =
        bond.putPrice * Math.exp(-bondYield * timeToPut) * this.normalCDF(-d2) -
        straightBondPrice * this.normalCDF(-d1);

      optionValue = Math.max(0, optionValue); // Positive for bondholder
    }

    const optionAdjustedSpread = this.calculateOAS(
      bond,
      marketData.yieldCurve,
      straightBondPrice - optionValue,
      marketData.volatility
    );
  ...
  static analyzePortfolio(
    ...
    const weightedAverageYield = bondAnalyses.reduce(
      (sum, item) => sum + item.contribution.yield,
      0
    );
    const portfolioDuration = bondAnalyses.reduce(
      (sum, item) => sum + item.contribution.duration,
      0
    );
    const portfolioConvexity = bondAnalyses.reduce(
      (sum, item) => sum + item.contribution.convexity,
      0
    );
    ...
    const scenarios = scenarioShifts.map((shift) => {
      ...
      for (const item of bondAnalyses) {
        ...
        const convexityEffect =
          0.5 * item.pricing.convexity * Math.pow(shiftDecimal, 2) * item.marketValue;
  ...
  private static calculateAverageCreditRating(
    ...
    return Object.entries(ratings).sort(([, a], [, b]) => b - a)[0]?.[0] ?? 'Unrated';
  }
}
