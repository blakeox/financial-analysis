  ...
  static forecast(input: z.infer<typeof ScenarioInputSchema>): EbitdaForecastResult {
    ...
    for (let monthOffset = 1; monthOffset <= forecastPeriodMonths; monthOffset++) {
      const forecastDate = new Date(
        ...
        baselineDate.getMonth() + monthOffset
      );
      ...
      const totalBillableHours = activeEmployees.reduce((sum, emp) => {
        ...
        const adjustedHours = new Decimal(emp.billableHoursPerMonth).times(
          monthlyGrowth.pow(monthOffset)
        );
      ...
      let projectedOpEx = Number(
        baseOpEx.times(opExGrowthFactor).times(inflationFactor).toFixed(2)
      );
    ...
    const revenueGrowth =
      lastForecast && baselineFinancials.revenue > 0
