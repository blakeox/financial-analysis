...
export const OptionAnalysisInputSchema = z.object({
  ...
  marketData: z.object({
    ...
    dividendSchedule: z.array(z.object({
      exDate: z.string(),
  ...
  analysis: z.object({
    ...
    pricingModel: z.enum(['black-scholes', 'black-scholes-merton', 'binomial', 'monte-carlo']).default('black-scholes'),
...
export class OptionsPricingAnalyzer {
  ...
  static analyze(input: OptionAnalysisInput): OptionPricingResult {
    ...
    // Calculate Greeks
    const greeks = analysis.includeGreeks 
      ? this.calculateGreeks(option)
      : this.getEmptyGreeks();
    ...
    const { breakevenPoints, maxProfit, maxLoss } = this.calculateBreakevenAnalysis(option, theoreticalPrice);
    ...
    return {
      ...
      // Volatility Analysis
      impliedVolatility: impliedVolatility ? Number(new Decimal(impliedVolatility).toDecimalPlaces(6)) : undefined,
      ...
      // Risk Metrics
      probabilityOfProfit: Number(new Decimal(probabilities.probabilityOfProfit).toDecimalPlaces(4)),
  ...
  private static blackScholesPrice(option: Option): number {
    const { underlyingPrice: S, strikePrice: K, timeToExpiry: T, riskFreeRate: r, volatility: vol } = option;
  ...
  private static binomialPrice(option: Option, steps: number): number {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      optionValues[i] = option.type === 'call' 
        ? Math.max(0, ST - K)
        : Math.max(0, K - ST);
    }
    ...
    for (let step = steps - 1; step >= 0; step--) {
      for (let i = 0; i <= step; i++) {
        const exerciseValue = option.style === 'american' 
          ? this.calculateExerciseValue(S * Math.pow(u, step - i) * Math.pow(d, i), K, option.type)
          : 0;
          
        const holdValue = discount * (p * optionValues[i] + (1 - p) * optionValues[i + 1]);
        
        optionValues[i] = option.style === 'american' 
          ? Math.max(exerciseValue, holdValue)
          : holdValue;
      }
    }
   ...
   */
  private static calculateExerciseValue(spotPrice: number, strikePrice: number, optionType: OptionType): number {
  ...
  private static calculateGreeks(option: Option): Greeks {
    ...
    const upPrice = this.blackScholesMertonPrice({ ...option, underlyingPrice: option.underlyingPrice + dS });
    const downPrice = this.blackScholesMertonPrice({ ...option, underlyingPrice: option.underlyingPrice - dS });
    const delta = (upPrice - downPrice) / (2 * dS);
    
    const dT = 1 / 365; // 1 day change
    const timePrice = this.blackScholesMertonPrice({ ...option, timeToExpiry: Math.max(0.001, option.timeToExpiry - dT) });
    const theta = timePrice - basePrice; // Note: negative for time decay
    
    const dVol = 0.01; // 1% volatility change
    const volUpPrice = this.blackScholesMertonPrice({ ...option, volatility: option.volatility + dVol });
    const volDownPrice = this.blackScholesMertonPrice({ ...option, volatility: option.volatility - dVol });
    const vega = (volUpPrice - volDownPrice) / (2 * dVol);
    
    const dR = 0.0001; // 1bp interest rate change
    const rateUpPrice = this.blackScholesMertonPrice({ ...option, riskFreeRate: option.riskFreeRate + dR });
    const rateDownPrice = this.blackScholesMertonPrice({ ...option, riskFreeRate: option.riskFreeRate - dR });
    ...
    // Vanna (dDelta/dVol)
    const deltaUp = this.calculateSingleGreek(option, 'delta', { volatility: option.volatility + dVol });
    const deltaDown = this.calculateSingleGreek(option, 'delta', { volatility: option.volatility - dVol });
    const vanna = (deltaUp - deltaDown) / (2 * dVol);
    
    // Charm (dDelta/dTime)
    const deltaTime = this.calculateSingleGreek(option, 'delta', { timeToExpiry: Math.max(0.001, option.timeToExpiry - dT) });
    const charm = deltaTime - delta; // Change in delta per day
    
    // Vomma (dVega/dVol)
    const vegaUp = this.calculateSingleGreek(option, 'vega', { volatility: option.volatility + dVol });
    const vegaDown = this.calculateSingleGreek(option, 'vega', { volatility: option.volatility - dVol });
  ...
  private static calculateSingleGreek(
    ...
    switch (greek) {
      case 'delta': {
        const dS = 0.01;
        const upPrice = this.blackScholesMertonPrice({ ...modifiedOption, underlyingPrice: modifiedOption.underlyingPrice + dS });
        const downPrice = this.blackScholesMertonPrice({ ...modifiedOption, underlyingPrice: modifiedOption.underlyingPrice - dS });
        return (upPrice - downPrice) / (2 * dS);
      }
      case 'vega': {
        const dVol = 0.01;
        const upPrice = this.blackScholesMertonPrice({ ...modifiedOption, volatility: modifiedOption.volatility + dVol });
        const downPrice = this.blackScholesMertonPrice({ ...modifiedOption, volatility: modifiedOption.volatility - dVol });
  ...
  private static calculateSpeed(option: Option): number {
    const dS = 0.01;
    const upGamma = this.calculateGamma({ ...option, underlyingPrice: option.underlyingPrice + dS });
    const downGamma = this.calculateGamma({ ...option, underlyingPrice: option.underlyingPrice - dS });
    return (upGamma - downGamma) / (2 * dS);
  }

  private static calculateColor(option: Option): number {
    const dT = 1 / 365;
    const timeGamma = this.calculateGamma({ ...option, timeToExpiry: Math.max(0.001, option.timeToExpiry - dT) });
  ...
  private static calculateVomma(option: Option): number {
    const dVol = 0.01;
    const upVega = this.calculateSingleGreek(option, 'vega', { volatility: option.volatility + dVol });
    const downVega = this.calculateSingleGreek(option, 'vega', { volatility: option.volatility - dVol });
  ...
  private static getEmptyGreeks(): Greeks {
    return {
      ...
      vanna: 0, charm: 0, vomma: 0, ultima: 0, speed: 0, color: 0, zomma: 0,
    };
  }
  ...
  private static calculateProbabilities(option: Option): {
    ...
    const probabilityITM = option.type === 'call' 
      ? this.normalCDF(d2)
      : this.normalCDF(-d2);
    ...
    const breakevenPrice = option.type === 'call' 
      ? K + this.blackScholesMertonPrice(option) 
      : K - this.blackScholesMertonPrice(option);
    
    const dProfit = (Math.log(S / breakevenPrice) + (r - q - 0.5 * vol * vol) * T) / (vol * Math.sqrt(T));
    const probabilityOfProfit = option.type === 'call' 
      ? this.normalCDF(dProfit)
      : this.normalCDF(-dProfit);
    
    // Probability of touching strike price
    const probabilityTouch = 2 * this.normalCDF(
      (Math.log(K / S) + 0.5 * vol * vol * T) / (vol * Math.sqrt(T))
    );
   ...
   * Calculate breakeven analysis
   */
  private static calculateBreakevenAnalysis(option: Option, premium: number): {
    breakevenPoints: number[];
   ...
   * Calculate time decay analysis
   */
  private static calculateTimeDecayAnalysis(option: Option, days: number[]): Array<{
    daysToExpiry: number;
    theoreticalPrice: number;
    theta: number;
    timeValue: number;
    ...
    for (const dayCount of days) {
      ...
      const nextDayPrice = this.blackScholesMertonPrice({ ...modifiedOption, timeToExpiry: nextDayTime });
  ...
  static analyzeStrategy(
    ...
    // Risk metrics
    const sharpeRatio = this.calculateSharpeRatio(payoffDiagram, positions[0]?.option.riskFreeRate ?? 0.05);
   ...
   */
  private static analyzeStrategyRisk(payoffDiagram: Array<{ underlyingPrice: number; payoff: number }>) {
    ...
    for (let i = 1; i < payoffDiagram.length; i++) {
      ...
      if ((current.payoff >= 0 && previous.payoff < 0) || (current.payoff < 0 && previous.payoff >= 0)) {
        // Interpolate to find exact breakeven point
        const ratio = -previous.payoff / (current.payoff - previous.payoff);
        const breakevenPrice = previous.underlyingPrice + ratio * (current.underlyingPrice - previous.underlyingPrice);
   ...
   */
  private static calculateExpectedValue(payoffDiagram: Array<{ underlyingPrice: number; payoff: number }>): number {
   ...
   */
  private static calculateMaximumDrawdown(payoffDiagram: Array<{ underlyingPrice: number; payoff: number }>): number {
    let maxDrawdown = 0;
    let peak = -Infinity;
