...
import type {
  ...
  ScenarioAnalysis,
  RiskMetrics,
} from '../../types/options-pricing-result.js';
...
export class OptionsPricingAnalyzer {
  ...
  private static calculateOptionPricing(
    input: OptionsPricingInput,
    ...
    if (input.optionType === 'call') {
      // Call option
      theoreticalValue =
        S.mul(Math.exp(-q.toNumber() * T.toNumber()))
          .mul(Nd1)
          .toNumber() -
        K.mul(Math.exp(-r.toNumber() * T.toNumber()))
          .mul(Nd2)
          .toNumber();
    ...
    const probabilityProfit = this.calculateProbabilityProfit(
      input,
      breakEvenPrice,
      timeToExpiration
    );
    ...
    const maxProfit =
      input.optionType === 'call' ? Infinity : (input.strikePrice - premium) * input.contractSize;
  ...
  private static calculateGreeks(
    ...
    // Delta
    const delta =
      input.optionType === 'call' ? Math.exp(-q * T) * Nd1 : -Math.exp(-q * T) * Nminusd1;
  ...
  private static performScenarioAnalysis(
    ...
    for (
      let vol = input.volatility * 0.5;
      vol <= input.volatility * 1.5;
      vol += input.volatility * 0.1
    ) {
    ...
    const timeDecayScenarios: Array<{
  ...
  private static assessRisk(
    ...
    const expectedReturn =
      pricing.probabilityProfit * pricing.maxProfit - probabilityOfLoss * pricing.maxLoss;

    const volatilityRisk =
      input.volatility > 0.5 ? 'High' : input.volatility > 0.3 ? 'Medium' : 'Low';
    const timeDecayRisk =
      Math.abs(greeks.theta) > pricing.theoreticalValue * 0.05
        ? 'High'
        : Math.abs(greeks.theta) > pricing.theoreticalValue * 0.02
          ? 'Medium'
          : 'Low';
  ...
  private static generateInsights(
    ...
    insights.push(
      `Option is ${pricing.moneyness} (${pricing.inMoneyAmount > 0 ? `$${pricing.inMoneyAmount.toFixed(2)} in the money` : 'out of the money'})`
    );
    insights.push(
      `Delta: ${greeks.delta.toFixed(4)} - ${Math.abs(greeks.delta * 100).toFixed(2)}% price sensitivity`
    );
  ...
  private static calculateProbabilityProfit(
    input: OptionsPricingInput,
    breakEven: number,
    T: number
  ): number {
    ...
    const d =
      (Math.log(S / breakEven) + (r - q - (sigma * sigma) / 2) * T) / (sigma * Math.sqrt(T));
