...
export function analyze(input: AutoLoanInput): AutoLoanResult {
	...
	const earlyPayoffScenarios = generateEarlyPayoffScenarios(
		paymentSchedule,
		input.loanTermMonths
	);
...
function calculateCostBreakdown(input: AutoLoanInput): AutoLoanCostBreakdown {
	...
	const gapInsurance = input.includeGapInsurance ? new Decimal(input.gapInsuranceCost) : new Decimal(0);
	const extendedWarranty = input.includeExtendedWarranty ? new Decimal(input.extendedWarrantyCost) : new Decimal(0);
