...
describe('Auto Loan Analysis Engine', () => {
  describe('Basic Loan Calculations', () => {
    ...
    it('should return all required fields in the analysis result', () => {
      ...
      expect(result).toHaveProperty('earlyPayoffScenarios');
    });
  ...
  describe('Payment Schedule (Amortization)', () => {
    ...
    it('should have zero or near-zero final balance', () => {
      ...
      const finalBalance = parseFloat(
        result.paymentSchedule[result.paymentSchedule.length - 1].balance
      );
    ...
    it('should increase principal portion over time', () => {
      ...
      const lastPrincipal = parseFloat(
        result.paymentSchedule[result.paymentSchedule.length - 1].principal
      );
    ...
    it('should decrease interest portion over time', () => {
      ...
      const lastInterest = parseFloat(
        result.paymentSchedule[result.paymentSchedule.length - 1].interest
      );
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle maximum allowed interest rate', () => {
      const input = createBasicLoanInput({ interestRate: 0.5 });
      const result = analyze(input);
