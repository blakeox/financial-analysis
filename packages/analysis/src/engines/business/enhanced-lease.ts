...
export class EnhancedLeaseAnalyzer {
  ...
  static analyze(input: z.infer<typeof EnhancedLeaseInputSchema>): EnhancedLeaseAnalysisResult {
    ...
    const basePayment = validated.leaseType === 'equipment' 
   ...
   */
  private static calculateEquipmentPayment(input: EnhancedLeaseInput): { basePayment: number; interestRate: number } {
  ...
  private static generatePaymentSchedule(
    ...
    for (let month = 1; month <= input.termMonths; month++) {
      ...
      const effectiveRate = month === 1 ? input.annualRate : 
        (cumulativePaid / input.principal - 1) * (12 / month);
  ...
  private static applyEscalation(basePayment: number, month: number, escalation?: EnhancedLeaseInput['escalation']): number {
  ...
  private static calculatePercentageRent(percentageRent?: EnhancedLeaseInput['percentageRent']): number {
  ...
  private static calculateFinancialMetrics(schedule: EnhancedLeasePaymentItem[], input: EnhancedLeaseInput): FinancialMetrics {
  ...
  private static analyzeEscalations(escalation: NonNullable<EnhancedLeaseInput['escalation']>, schedule: EnhancedLeasePaymentItem[]): EscalationSummary {
  ...
  private static analyzeRenewalOptions(input: EnhancedLeaseInput, metrics: FinancialMetrics): RenewalAnalysis[] {
    return input.renewalOptions.map((option, index) => ({
      ...
      totalOptionCost: metrics.averageMonthlyPayment * (1 + option.rateAdjustment) * option.termMonths,
      presentValue: 0, // Would calculate based on discount rate
      effectiveRate: input.annualRate + option.rateAdjustment,
    }));
  }
  
  private static analyzePurchaseOption(input: EnhancedLeaseInput): PurchaseOptionAnalysis | undefined {
  ...
  private static analyzeLeaseVsBuy(input: EnhancedLeaseInput, metrics: FinancialMetrics): LeaseVsBuyAnalysis | undefined {
    ...
    const loanPayment = purchasePrice * (monthlyLoanRate * Math.pow(1 + monthlyLoanRate, loanTerm)) / 
  ...
  private static analyzeRisk(input: EnhancedLeaseInput, metrics: FinancialMetrics): RiskAnalysis {
    const earlyTerminationCost = input.earlyTermination?.allowed ? 
      (input.earlyTermination.penaltyAmount || metrics.averageMonthlyPayment * (input.earlyTermination.penaltyMonths || 3)) : 
      metrics.totalCost;
  ...
  private static generateInsights(input: EnhancedLeaseInput, metrics: FinancialMetrics, risk: RiskAnalysis) {
    ...
    if (input.escalation?.type !== 'none') {
      recommendations.push('Monitor escalation clauses to ensure they align with market conditions');
    }
    ...
    return {
      ...
      flexibilityRating: risk.flexibilityScore > 75 ? 'High' : risk.flexibilityScore > 50 ? 'Medium' : 'Low',
      recommendations,
    };
  }
