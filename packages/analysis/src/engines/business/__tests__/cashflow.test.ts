...
describe('CashFlowAnalyzer - Core Valuation Metrics', () => {
  ...
  describe('Comprehensive Analysis Output', () => {
    it('generates cash flow summary', () => {
      const result = CashFlowAnalyzer.analyze(createCoreInput());
      expect(result.cashFlowSummary).toBeDefined();
      expect(result.cashFlowSummary.totalInflows).toBeGreaterThan(0);
      expect(result.cashFlowSummary.totalOutflows).toBeGreaterThan(0);
      expect(result.cashFlowSummary.netCashFlow).toBeDefined();
    });

    it('generates detailed cash flows', () => {
      const result = CashFlowAnalyzer.analyze(createCoreInput());
      expect(result.detailedCashFlows).toBeDefined();
      expect(result.detailedCashFlows.length).toBeGreaterThan(0);
      expect(result.detailedCashFlows[0].presentValue).toBeDefined();
    });

    it('generates sensitivity analysis when requested', () => {
      const input = createCoreInput({
        analysis: { includeSensitivity: true },
      });
      const result = CashFlowAnalyzer.analyze(input);
  ...
  describe('Comprehensive Analysis', () => {
    it('should return all required fields in the analysis result', () => {
      ...
      expect(result).toHaveProperty('profitabilityIndex');
      expect(result).toHaveProperty('equivalentAnnuity');
      expect(result).toHaveProperty('detailedCashFlows');
      expect(result).toHaveProperty('cashFlowSummary');
    });
  });
});
