...
export class RevenueForecastEngine {
  ...
  static analyze(input: RevenueForecastInput): RevenueForecastResult {
    ...
    for (let month = 1; month <= months; month++) {
      ...
      const growthVsPreviousMonth =
        month > 1
          ? ((totalMonthRevenue - monthlyForecasts[month - 2]!.totalRevenue) /
              monthlyForecasts[month - 2]!.totalRevenue) *
            100
          : 0;

      const growthVsYearAgo =
        month > 12
          ? ((totalMonthRevenue - monthlyForecasts[month - 13]!.totalRevenue) /
              monthlyForecasts[month - 13]!.totalRevenue) *
            100
          : undefined;
    ...
    const compoundMonthlyGrowthRate =
      firstMonthRevenue > 0
   ...
   */
  private static calculateStreamBreakdown(streams: RevenueStream[], forecasts: MonthlyForecast[]) {
    return streams.map((stream) => {
      const totalRevenue = forecasts.reduce(
        (sum, month) => sum + (month.revenueByStream[stream.name] || 0),
        0
      );
  ...
  private static generateInsights(
    ...
    const peakMonth =
      forecasts.reduce(
        (max, curr) => (curr.totalRevenue > (max?.totalRevenue || 0) ? curr : max),
        forecasts[0]
      ) || forecasts[0];
    const lowestMonth =
      forecasts.reduce(
        (min, curr) => (curr.totalRevenue < (min?.totalRevenue || Infinity) ? curr : min),
        forecasts[0]
      ) || forecasts[0];

    insights.push(
      `Revenue peaks in ${peakMonth!.monthName} at ${this.formatCurrency(peakMonth!.totalRevenue)} and is lowest in ${lowestMonth!.monthName} at ${this.formatCurrency(lowestMonth!.totalRevenue)}.`
    );

    // Seasonality insight
    const seasonalVariation =
      ((peakMonth!.totalRevenue - lowestMonth!.totalRevenue) / lowestMonth!.totalRevenue) * 100;
    if (seasonalVariation > 30) {
      insights.push(
        `⚠️ High seasonality (${seasonalVariation.toFixed(0)}% variation) - ensure sufficient cash reserves for low months.`
      );
    }
    ...
    if (topStream.percentOfTotal > 70) {
      insights.push(
        `⚠️ Revenue concentration: ${topStream.name} represents ${topStream.percentOfTotal.toFixed(0)}% of total revenue.`
      );
    }
    ...
    if (customerMetrics && customerMetrics.endingCustomers !== undefined) {
      ...
      insights.push(
        `Customer base grows from ${startingCustomers.toFixed(0)} to ${customerMetrics.endingCustomers.toFixed(0)} customers over the forecast period.`
      );

      if (customerMetrics.totalChurned > customerMetrics.totalAcquired * 0.5) {
        insights.push(
          `⚠️ High churn: You're losing ${((customerMetrics.totalChurned / customerMetrics.totalAcquired) * 100).toFixed(0)}% as many customers as you acquire.`
        );
      }
    }
  ...
  private static generateRecommendations(
    ...
    if (totalGrowth < 10) {
      recommendations.push(
        `📈 Accelerate growth: Current ${totalGrowth.toFixed(1)}% growth is modest. Target 10%+ for healthy businesses.`
      );
    }
    ...
    if (streamBreakdown.length === 1) {
      recommendations.push(
        `🎯 Diversify revenue: Add new revenue streams to reduce dependency on single source.`
      );
    }
    ...
    if (topStream.percentOfTotal > 70) {
      recommendations.push(
        `⚠️ Reduce concentration: ${topStream.name} is ${topStream.percentOfTotal.toFixed(0)}% of revenue. Add complementary streams.`
      );
    }
    ...
    if (slowGrowthStreams.length > 0) {
      recommendations.push(
        `🚀 Invest in growth for: ${slowGrowthStreams.map((s) => s.name).join(', ')}`
      );
    }
    ...
    if (input.monthlyChurnRate && input.monthlyChurnRate > 5) {
      recommendations.push(
        `🔄 Reduce churn from ${input.monthlyChurnRate.toFixed(1)}% to <5% to accelerate revenue growth.`
      );
  ...
  private static generateRisks(
    ...
    const avgGrowthRate =
      input.revenueStreams.reduce((sum, s) => sum + s.growthRate, 0) / input.revenueStreams.length;
    if (avgGrowthRate > 30) {
      risks.push(
        `⚠️ Aggressive ${avgGrowthRate.toFixed(0)}% growth assumption - actual results may vary significantly.`
      );
    }
    ...
    if (decliningStreams.length > 0) {
      risks.push(
        `📉 Revenue decline risk in: ${decliningStreams.map((s) => `${s.name} (${s.growthRate.toFixed(0)}%)`).join(', ')}`
      );
    }
    ...
    if (input.newCustomersPerMonth && input.existingCustomers && input.monthlyChurnRate) {
      const netGrowthRate =
        (input.newCustomersPerMonth / input.existingCustomers - input.monthlyChurnRate / 100) * 100;
      if (netGrowthRate < 2) {
        risks.push(
          `⚠️ Low net customer growth (${netGrowthRate.toFixed(1)}%/month) may limit revenue upside.`
        );
      }
    }
    ...
    if (input.forecastMonths > 12) {
      risks.push(
        `📅 Long forecast horizon (${input.forecastMonths} months) increases uncertainty. Review and update quarterly.`
      );
    }
