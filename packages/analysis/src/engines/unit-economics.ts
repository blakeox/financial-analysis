...
export class UnitEconomicsEngine {
  ...
  static analyze(input: UnitEconomicsInput): UnitEconomicsResult {
    // Defaults
    const discountRate = input.discountRate || 0.1; // 10% annual = 0.833% monthly
    ...
    const contributionMarginPerCustomer =
      grossMarginDollars - input.variableServicingCostPerCustomer;
    ...
    const paybackPeriodMonths =
      contributionMarginPerCustomer > 0 ? cac / contributionMarginPerCustomer : 999;
  ...
  private static generateCohortAnalysis(
    ...
    for (let month = 0; month <= months; month++) {
      ...
      cohort.push({
        ...
        lifetimeValue:
          (prevCumRevenue + monthRevenue - prevCumCosts - variableCosts - acquisitionCost) / 100,
      });
  ...
  private static generateInsights(
    ...
    if (ltvCacRatio >= 5) {
      insights.push(
        `🎯 Exceptional LTV:CAC ratio of ${ltvCacRatio.toFixed(1)}:1 - You have strong product-market fit and can invest more in customer acquisition.`
      );
    } else if (ltvCacRatio >= 3) {
      insights.push(
        `✅ Healthy LTV:CAC ratio of ${ltvCacRatio.toFixed(1)}:1 - Your unit economics are sustainable.`
      );
    } else if (ltvCacRatio >= 2) {
      insights.push(
        `⚠️ Marginal LTV:CAC ratio of ${ltvCacRatio.toFixed(1)}:1 - Focus on improving retention or reducing CAC.`
      );
    } else {
      insights.push(
        `🚨 Critical LTV:CAC ratio of ${ltvCacRatio.toFixed(1)}:1 - Current unit economics are not sustainable. Urgent action needed.`
      );
    }
    ...
    if (paybackMonths <= 6) {
      insights.push(
        `💰 Excellent payback period of ${paybackMonths.toFixed(1)} months - You recover acquisition costs quickly.`
      );
    } else if (paybackMonths <= 12) {
      insights.push(
        `✅ Good payback period of ${paybackMonths.toFixed(1)} months - Industry standard for most SaaS businesses.`
      );
    } else if (paybackMonths <= 18) {
      insights.push(
        `⚠️ Long payback period of ${paybackMonths.toFixed(1)} months - Consider ways to accelerate revenue or reduce CAC.`
      );
    } else {
      insights.push(
        `🚨 Very long payback period of ${paybackMonths.toFixed(1)} months - This creates significant cash flow risk.`
      );
    }
    ...
    if (churnRate <= 3) {
      insights.push(
        `🎯 Excellent retention with ${churnRate.toFixed(1)}% monthly churn - Your product has strong stickiness.`
      );
    } else if (churnRate <= 5) {
      insights.push(
        `✅ Good retention with ${churnRate.toFixed(1)}% monthly churn - Within healthy range for most businesses.`
      );
    } else if (churnRate <= 7) {
      insights.push(
        `⚠️ High churn rate of ${churnRate.toFixed(1)}% per month - Focus on improving customer success and product value.`
      );
    } else {
      insights.push(
        `🚨 Critical churn rate of ${churnRate.toFixed(1)}% per month - Retention issues are severely limiting growth.`
      );
    }
    ...
    if (grossMargin >= 80) {
      insights.push(
        `💎 Outstanding ${grossMargin.toFixed(1)}% gross margin - You have a highly scalable business model.`
      );
    } else if (grossMargin >= 70) {
      insights.push(
        `✅ Strong ${grossMargin.toFixed(1)}% gross margin - Good foundation for profitability.`
      );
    } else if (grossMargin >= 60) {
      insights.push(
        `⚠️ Moderate ${grossMargin.toFixed(1)}% gross margin - Look for ways to reduce COGS or increase prices.`
      );
    } else {
      insights.push(
        `🚨 Low ${grossMargin.toFixed(1)}% gross margin - High COGS may limit profitability. Review pricing and cost structure.`
      );
    }
    ...
    if (contributionMargin > 0) {
      insights.push(
        `Each customer contributes $${contributionMargin.toFixed(2)}/month after variable costs - this drives your profitability.`
      );
    } else {
      insights.push(
        `🚨 Negative contribution margin of $${contributionMargin.toFixed(2)}/month - You lose money on every customer. Urgent pricing/cost review needed.`
      );
    }
    ...
    if (netRevenueRetention >= 110) {
      insights.push(
        `🚀 Exceptional ${netRevenueRetention.toFixed(0)}% net revenue retention - Negative churn through expansion revenue!`
      );
    } else if (netRevenueRetention >= 100) {
      insights.push(
        `💰 Strong ${netRevenueRetention.toFixed(0)}% net revenue retention - Expansion revenue offsets churn.`
      );
    } else if (netRevenueRetention >= 90) {
      insights.push(
        `Net revenue retention of ${netRevenueRetention.toFixed(0)}% - Focus on upsells and cross-sells to reach 100%+.`
      );
    }
  ...
  private static generateRecommendations(
    ...
    if (ltvCacRatio < 3) {
      recommendations.push(
        `📈 Improve LTV:CAC Ratio: Target 3:1 or better. Current: ${ltvCacRatio.toFixed(1)}:1`
      );

      if (churnRate > 5) {
        recommendations.push(
          `   • Reduce churn from ${churnRate.toFixed(1)}% to <5% by improving onboarding and customer success`
        );
      }

      if (cac > ltv * 0.5) {
        recommendations.push(
          `   • Reduce CAC by optimizing marketing channels and improving conversion rates`
        );
      }
    ...
    if (paybackMonths > 12) {
      recommendations.push(
        `⏱️ Accelerate Payback: Target <12 months. Current: ${paybackMonths.toFixed(1)} months`
      );
    ...
    if (churnRate > 5) {
      recommendations.push(
        `🔄 Reduce Churn: Target <5% monthly. Current: ${churnRate.toFixed(1)}%`
      );
    ...
    if (grossMargin < 70) {
      recommendations.push(
        `📊 Improve Gross Margin: Target 70%+. Current: ${grossMargin.toFixed(1)}%`
      );
    ...
    if (ltvCacRatio >= 3 && paybackMonths <= 12) {
      recommendations.push(
        `🚀 Scale Opportunity: Your unit economics are strong - consider increasing marketing spend to accelerate growth`
      );
      recommendations.push(
        `   • Current CAC of $${cac.toFixed(2)} is sustainable with LTV of $${ltv.toFixed(2)}`
      );
      recommendations.push(
        `   • You can afford to spend up to $${(ltv / 3).toFixed(2)} per customer while maintaining healthy ratios`
      );
    }
  ...
  private static generateWarnings(
    ...
    if (ltvCacRatio < 1) {
      warnings.push(
        `🚨 CRITICAL: You're losing money on every customer. LTV ($${ltv.toFixed(2)}) < CAC ($${cac.toFixed(2)}). Immediate action required.`
      );
    }

    if (paybackMonths > 24) {
      warnings.push(
        `⚠️ CASH FLOW RISK: ${paybackMonths.toFixed(1)}-month payback creates significant working capital needs.`
      );
    }

    if (churnRate > 10) {
      warnings.push(
        `🚨 CRITICAL CHURN: ${churnRate.toFixed(1)}% monthly churn means you lose half your customers every 7 months.`
      );
    }

    if (churnRate > 7 && paybackMonths > 12) {
      warnings.push(
        `⚠️ DOUBLE RISK: High churn + long payback = you may lose customers before recovering acquisition costs.`
      );
    }
